...
.sheet-wrap{width:100%;max-width:680px;box-shadow:var(--shadow-lg);border-radius:3px;overflow:hidden;animation:sheetIn 0.4s ease;transform-origin:top center}
...
/* ══ RESUME TEMPLATES ══ */
/* 1. MODERN */
.tpl-modern{background:#fff;font-family:'Plus Jakarta Sans',sans-serif;font-size:10px;color:#1a1d23;min-height:500px}
...
/* 2. CORPORATE */
.tpl-corp{background:#fff;font-family:'Plus Jakarta Sans',sans-serif;font-size:10px;color:#1a1d23;min-height:500px}
...
/* 3. SIMPLE */
.tpl-simple{background:#fff;font-family:'Plus Jakarta Sans',sans-serif;font-size:10px;color:#2d3748;min-height:500px;padding:24px 28px}
...
/* 4. ELEGANT */
.tpl-elegant{background:#fff;font-family:'Playfair Display',serif;font-size:10px;color:#2c2c2c;min-height:500px}
...
/* 5. CREATIVE */
.tpl-creative{background:#fff;font-family:'Plus Jakarta Sans',sans-serif;font-size:10px;color:#1a1a2e;min-height:500px}
...
const TEMPLATES = [
  ...
  { id: "simple", name: "Simple", tag: "Clean", desc: "Minimal & distraction-free" },
  { id: "elegant", name: "Elegant", tag: "Premium", desc: "Gold accents with serif typography" },
  { id: "creative", name: "Creative", tag: "Modern", desc: "Purple gradient for creative roles" },
];
...
function ResumeContent({ data, tpl }) {
  ...
  if (tpl === "modern") return (
    <div className="tpl-modern">
      <div className="r-header">
        ...
        <div className="r-contacts">{contacts.map((c,i)=><span key={i} className="r-contact-item">◆ {c}</span>)}</div>
      </div>
  ...
  if (tpl === "corp") return (
    <div className="tpl-corp">
      <div className="r-header">
        ...
        <div className="r-header-right"><div className="r-contacts">{contacts.map((c,i)=><span key={i}>{c}</span>)}</div></div>
      </div>
      <div className="r-body">
  ...
  if (tpl === "simple") return (
    <div className="tpl-simple">
      <div className="r-name">{p.name||"Your Name"}</div>
      {p.title&&<div className="r-title">{p.title}</div>}
      <div className="r-contacts">{contacts.map((c,i)=><span key={i}>{c}</span>)}</div>
  ...
  if (tpl === "creative") return (
    <div className="tpl-creative">
      <div className="r-header">
        ...
        <div className="r-name">{p.name||"Your Name"}</div>
        {p.title&&<div className="r-title">{p.title}</div>}
        <div className="r-contacts">{contacts.map((c,i)=><span key={i} className="r-contact-pill">{c}</span>)}</div>
      </div>
      <div className="r-body">
        <div className="r-sidebar">
          {p.summary&&<div className="r-sec"><div className="r-sec-title">About</div><div className="r-summary">{p.summary}</div></div>}
...
async function genWord(d) {
  ...
  const url=URL.createObjectURL(blob);const a=document.createElement("a");a.href=url;a.download=`${d.personal.name||"resume"}_Resume.docx`;a.click();URL.revokeObjectURL(url);
}
...
export default function App() {
  ...
  const handleDownload = async (f) => {
    ...
    finally{setDlLoading(null);setShowSuccess(false);}
  };
