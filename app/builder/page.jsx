...
function ResumeContent({ data, tpl }) {
  ...
  if (tpl === "modern") return (
    <div className="tpl-modern">
      <div className="r-header">
        ...
        <div className="r-contacts">{contacts.map((c,i)=><span key={i} className="r-contact-item"><span style={{marginRight:3}}>{contactIcon(c)}</span>{c}</span>)}</div>
      </div>
      <div className="r-body">
        ...
        <div className="r-main">
          {exps.length>0&&<div className="r-sec"><div className="r-sec-title">Experience</div>
            {exps.map((e,i)=>{const bs=e.bullets?.split("\n").filter(b=>b.trim())||[];return(
              <div key={i} className="r-exp-item"><div className="r-exp-top"><span className="r-exp-role">{e.role}</span>{e.duration&&<span className="r-exp-dur">{e.duration}</span>}</div><div className="r-exp-co">{e.company}</div>{bs.map((b,j)=><div key={j} className="r-bullet"><span className="r-dot"/>{b.replace(/^[•\-*]\s*/,"")}</div>)}</div>
            )})}
          </div>}
        </div>
      </div>
  ...
  if (tpl === "corp") return (
    <div className="tpl-corp">
      <div className="r-header">
        ...
        <div className="r-header-right"><div className="r-contacts">{contacts.map((c,i)=><span key={i} className="r-contact-item"><span style={{marginRight:3}}>{contactIcon(c)}</span>{c}</span>)}</div></div>
      </div>
      <div className="r-body">
        ...
        <div className="r-two-col">
          ...
          <div>
            {exps.length>0&&<div className="r-sec"><div className="r-sec-title">Work Experience</div>
              {exps.map((e,i)=>{const bs=e.bullets?.split("\n").filter(b=>b.trim())||[];return(
                <div key={i} style={{marginBottom:14}}><div className="r-exp-role">{e.role}</div><div className="r-exp-co">{e.company}</div><div className="r-exp-dur">{e.duration}</div>{bs.map((b,j)=><div key={j} className="r-bullet"><span className="r-dot"/>{b.replace(/^[•\-*]\s*/,"")}</div>)}</div>
              )})}
            </div>}
          </div>
        </div>
  ...
  if (tpl === "simple") return (
    <div className="tpl-simple">
      <div className="r-name">{p.name||"Your Name"}</div>
      {p.title&&<div className="r-title">{p.title}</div>}
      <div className="r-contacts">{contacts.map((c,i)=><span key={i}><span style={{marginRight:3}}>{contactIcon(c)}</span>{c}</span>)}</div>
      {p.summary&&<><div className="r-sec-title">Summary</div><div className="r-summary">{p.summary}</div></>}
      {exps.length>0&&<><div className="r-sec-title" style={{marginTop:14}}>Experience</div>{exps.map((e,i)=>{const bs=e.bullets?.split("\n").filter(b=>b.trim())||[];return(<div key={i} className="r-exp-item"><div className="r-exp-top"><span className="r-exp-role">{e.role}</span><span className="r-exp-dur">{e.duration}</span></div><div className="r-exp-co">{e.company}</div>{bs.map((b,j)=><div key={j} className="r-bullet"><span className="r-dot">–</span>{b.replace(/^[•\-*]\s*/,"")}</div>)}</div>)})}</>}
    ...
    <div className="tpl-elegant">
      ...
      <div className="r-body">
        ...
        <div className="r-main">
          {exps.length>0&&<div className="r-sec"><div className="r-sec-title">Experience</div>
            {exps.map((e,i)=>{const bs=e.bullets?.split("\n").filter(b=>b.trim())||[];return(
              <div key={i} style={{marginBottom:14}}><div className="r-exp-role">{e.role}</div><div className="r-exp-co">{e.company}</div><div className="r-exp-dur">{e.duration}</div>{bs.map((b,j)=><div key={j} className="r-bullet"><span className="r-dot">◈</span>{b.replace(/^[•\-*]\s*/,"")}</div>)}</div>
            )})}
          </div>}
        </div>
      </div>
  ...
  if (tpl === "creative") return (
    <div className="tpl-creative">
      <div className="r-header">
        ...
        <div className="r-name">{p.name||"Your Name"}</div>
        {p.title&&<div className="r-title">{p.title}</div>}
        <div className="r-contacts">{contacts.map((c,i)=><span key={i} className="r-contact-pill"><span style={{marginRight:3}}>{contactIcon(c)}</span>{c}</span>)}</div>
      </div>
      <div className="r-body">
        <div className="r-sidebar">
          {p.summary&&<div className="r-sec"><div className="r-sec-title">About</div><div className="r-summary">{p.summary}</div></div>}
  ...
  const sidebarNew = (accentColor, tagBg, tagColor) => (
    <>
      {p.summary&&<div className="r-sec"><div className="r-sec-title">About</div><div className="r-summary">{p.summary}</div></div>}
  ...
  const mainExpNew = () => exps.length>0?(
    <div className="r-sec"><div className="r-sec-title">Work Experience</div>
      {exps.map((e,i)=><div key={i} className="r-exp-item">
        ...
        {(e.bullets?.split("\n").filter(b=>b.trim())||[]).map((b,j)=><div key={j} className="r-bullet"><span className="r-dot">◆</span><span>{b.replace(/^[•\-*]\s*/,"")}</span></div>)}
      </div>)}
    </div>
  ):null;
  ...
  if(tpl==="teal"||tpl==="dark"||tpl==="rose") return(
    <div className={`tpl-${tpl}`}>
      <div className="r-header"><div className="r-name">{p.name||"Your Name"}</div>{p.title&&<div className="r-title">{p.title}</div>}<div className="r-contacts">{contacts.map((c,i)=><span key={i} className="r-contact-item"><span style={{marginRight:3}}>{contactIcon(c)}</span>{c}</span>)}</div></div>
  ...
  if(tpl==="orange") return(
    <div className="tpl-orange">
      <div className="r-header"><div className="r-header-inner"><div><div className="r-name">{p.name||"Your Name"}</div>{p.title&&<div className="r-title">{p.title}</div>}</div><div className="r-contacts">{contacts.map((c,i)=><span key={i} className="r-contact-item"><span style={{marginRight:3}}>{contactIcon(c)}</span>{c}</span>)}</div></div></div>
  ...
  if(tpl==="minimal") return(
    <div className="tpl-minimal">
      ...
      {exps.length>0&&<><div className="r-sec-title">Experience</div>{exps.map((e,i)=><div key={i} className="r-exp-item"><div className="r-exp-top"><span className="r-exp-role">{e.role}</span><span className="r-exp-dur">{e.duration}</span></div><div className="r-exp-co">{e.company}</div>{(e.bullets?.split("\n").filter(b=>b.trim())||[]).map((b,j)=><div key={j} className="r-bullet"><span className="r-dot">—</span><span>{b.replace(/^[•\-*]\s*/,"")}</span></div>)}</div>)}</>}
  ...
  const generate = async () => {
    ...
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages",{
        ...
        body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:1600,messages:[{role:"user",content:`You are an expert resume writer. Polish this resume and return ONLY valid JSON, no markdown.\n\nReturn:\n{\n  "personal":{"name":"","title":"","email":"","phone":"","location":"","linkedin":"","summary":"2-3 powerful ATS-rich sentences with keywords"},\n  "experience":[{"id":1,"company":"","role":"","duration":"","bullets":"strong action verb bullet\\nquantified metric bullet\\nimpact bullet"}],\n  "education":[{"id":1,"school":"","degree":"","year":"","gpa":""}],\n  "skills":{"technical":"comma-separated","soft":"comma-separated","languages":"comma-separated"},\n  "certifications":"one per line"\n}\n\nRules: Start bullets with strong verbs (Led, Achieved, Delivered, Grew, Reduced, Managed). Add realistic metrics. Make summary ATS-optimized. Keep empty as empty string.\n\n${resumeText()}`}]})
      });
      const d2 = await res.json();
      const parsed = JSON.parse(d2.content?.[0]?.text.replace(/```json|```/g,"").trim()||"{}");
      setAiResume(parsed); setStep(1);
    } catch(e){ setAiResume(data); setStep(1); } finally { setGenerating(false); }
  };

  /* ATS ANALYSIS + SUGGESTIONS */
  const runAts = async () => {
    setAtsLoading(true); setAts(null); setSuggestions([]);
    try {
      const res = await fetch("https://api.anthropic.com/v1/messages",{
        method:"POST", headers:{"Content-Type":"application/json"},
        body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:1200,messages:[{role:"user",content:`Analyze this resume for ATS and provide suggestions. Return ONLY valid JSON:\n{\n  "score":<0-100>,\n  "strengths":["strength1","strength2","strength3"],\n  "improvements":["improvement1","improvement2","improvement3"],\n  "keywords_found":["kw1","kw2","kw3","kw4","kw5"],\n  "keywords_missing":["kw1","kw2","kw3"],\n  "suggestions":[\n    {"id":"s1","type":"enhance","field":"summary","label":"Enhance Professional Summary","description":"Your summary can be stronger. Suggested: add your years of experience and key achievement.","autoText":"Results-driven professional with proven track record of delivering impactful solutions and driving measurable business outcomes."},\n    {"id":"s2","type":"add","field":"skills","label":"Add In-Demand Skills","description":"Consider adding trending industry keywords to improve ATS match rate.","autoText":"Problem Solving, Cross-functional Collaboration, Data Analysis"},\n    {"id":"s3","type":"modify","field":"bullets","label":"Quantify Achievements","description":"Add specific numbers/percentages to your bullet points for stronger impact.","autoText":""},\n    {"id":"s4","type":"enhance","field":"certifications","label":"Add Certifications","description":"Certifications significantly boost ATS scores. Consider adding relevant ones.","autoText":""}\n  ],\n  "overall_tip":"One key insight for this candidate"\n}\n\nResume:\n${resumeText()}`}]})
      });
      const d2 = await res.json();
      const parsed = JSON.parse(d2.content?.[0]?.text.replace(/```json|```/g,"").trim()||"{}");
      setAts(parsed); setSuggestions(parsed.suggestions||[]); setStep(2);
    } catch(e){
      setAts({score:72,strengths:["Relevant work experience","Education section present","Skills listed"],improvements:["Add more quantified achievements","Expand professional summary","Include more industry keywords"],keywords_found:["Leadership","Communication","Management"],keywords_missing:["ROI","Agile","KPIs","Stakeholder"],overall_tip:"Focus on adding metrics to your bullet points to stand out."});
      setSuggestions([{id:"s1",type:"enhance",field:"summary",label:"Strengthen Summary",description:"Add your years of experience and key achievement to make your summary more impactful.",autoText:"Accomplished professional with 5+ years of experience delivering measurable results and leading cross-functional teams."},{id:"s2",type:"add",field:"skills",label:"Add High-Value Keywords",description:"Include trending skills to improve ATS match rate.",autoText:"Agile, KPI Tracking, Stakeholder Management, Data-Driven Decision Making"},{id:"s3",type:"modify",field:"bullets",label:"Quantify Your Impact",description:"Bullet points with numbers perform 40% better in ATS systems. Edit your bullet points to include percentages, numbers, or time frames.",autoText:""}]);
  ...
  const handleDownload = async (f) => {
    ...
    try {
      if(f==="pdf"){ if(!jsPdfReady||!window.jspdf){alert("PDF library loading, try again.");return;} genPDF(d,tpl); }
        ...
        if(!docxReady){ alert("Word library loading, please wait 3 seconds and try again."); return; }
        // FIX 5: wait for window.docx to be fully ready
        let tries = 0;
        while(!window.docx && tries < 10){ await new Promise(r=>setTimeout(r,500)); tries++; }
    ...
    // FIX 2: Do NOT reset paid or close modal — user can re-download in same session
  };
